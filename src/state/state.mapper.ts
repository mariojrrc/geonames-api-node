...
interface StateMapperOpts {
  db: unknown;
  stateModel: typeof stateModel;
  mapperOptions: Record<string, unknown>;
}

export default class StateMapper extends BaseMapper {
  constructor({
    db,
    stateModel: stateModelFn,
    mapperOptions,
  }: StateMapperOpts) {
    super(db, stateModelFn(), mapperOptions);
  }
